...
screenDisplay = (width_screen, height_screen) = (1500, 780)
FPS = 60 
gravity = 0.6
...
pygame.display.set_caption("Naruto Run - vivek&Harsh")
...
class naruto():
    def __init__(self, sx=-1, sy=-1):
        self.imgs, self.rect = load_sprite_sheet('naruto.png', 5, 1, sx*1.5, sy*1.5, -1)
        self.imgs1, self.rect1 = load_sprite_sheet(
            'dino_ducking.png', 2, 1, 59, sy, -1)
...
class Cactus(pygame.sprite.Sprite):
    def __init__(self, speed=5, sx=-1, sy=-1):
        ...
        # Adjust the width (sx) and height (sy) parameters to increase size
        self.imgs, self.rect = load_sprite_sheet(
            'cactus-small.png', 3, 1, sx * 1.5, sy * 1.5, -1)  # Doubling the size
...
class Cloud(pygame.sprite.Sprite):
    def __init__(self, x, y):
        pygame.sprite.Sprite.__init__(self, self.containers)
        self.image, self.rect = load_image('cloudi.png', int(90*80/42), 80, -1)
        self.speed = 5
        self.rect.left = x
        self.rect.top = y
...
def introduction_screen():
    ado_naruto = naruto(44, 47)
    ado_naruto.blinking = True
    starting_game = False
    ...
    logo, l_rect = load_image('narutooo.jpg', 1500, 780,-1)
    l_rect.centerx = width_screen // 2
    l_rect.centery = height_screen //2
    while not starting_game:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE or event.key == pygame.K_UP:
                        ado_naruto.jumping = True
                        ado_naruto.blinking = False
                        ado_naruto.movement[1] = -1*ado_naruto.jumpSpeed
        ...
        timerClock.tick(FPS)
        if ado_naruto.jumping == False and ado_naruto.blinking == False:
            starting_game = True
...
def gameplay():
    ...
    gmo_image, gmo_rect = load_image('gameover.png', 200, 200, -1)
    ...
    while not g_exit:
        ...
        while not g_Over:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            if gamer_naruto.rect.bottom == int(0.98 * height_screen):
                                gamer_naruto.jumping = True
                                if pygame.mixer.get_init() != None:
                                    soundOnJump.play()
                                gamer_naruto.movement[1] = - \
                                    1*gamer_naruto.jumpSpeed

                        if event.key == pygame.K_DOWN:
                            if not (gamer_naruto.jumping and gamer_naruto.dead):
                                gamer_naruto.ducking = True

                    if event.type == pygame.KEYUP:
                        if event.key == pygame.K_DOWN:
                            gamer_naruto.ducking = False
            for c in cactusan:
                c.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_naruto, c):
                    gamer_naruto.dead = True
                    if pygame.mixer.get_init() != None:
                        soundOnDie.play()

            for p in smallBird:
                p.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_naruto, p):
                    gamer_naruto.dead = True
                    if pygame.mixer.get_init() != None:
                        soundOnDie.play()
            ...
            skyClouds.update()
            new_grnd.update()
            score_boards.update(gamer_naruto.score)
            ...
            if gamer_naruto.dead:
                g_Over = True
                if gamer_naruto.score > highest_scores:
                    highest_scores = gamer_naruto.score
    ...
    while True:
        ...
        if hands:
            ...
            if fingerUp == [0, 0, 0, 0, 0]:
                ...
                cv2.putText(frame, 'Jumping', (440, 460),
                            cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
...
if __name__ == "__main__":
    # Run hand gesture recognition in a separate thread
    
    import threading
    game_thread = threading.Thread(target=hand_gesture_recognition)
    game_thread.start()
